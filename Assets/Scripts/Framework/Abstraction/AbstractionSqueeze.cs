...
public class AbstractionSqueeze : Abstraction
{	
	...
	float nMinXSize =  999f;
	float nMaxXSize = -999f;

	public AbstractionSqueeze()
	{
		dimension = new Vector3 (0.5f, 1f, 0);

		scale = 1f;
	}

	public AbstractionSqueeze(float scale)
	{
		dimension = new Vector3 (0.5f, 1f, 0);
	...
	public override Representation Process(Representation representation)
	{
		...
		result.size      = new Vector3 (nMaxXSize - nMinXSize, 0, 0);
		result.objectMin = new Vector3 (nMinXSize, 0, 0);
		result.objectMax = new Vector3 (nMaxXSize, 0, 0);

		//Debug.Log ("Squeeze : " + (Time.realtimeSinceStartup - startTime));
	...
	void createMesh(Graph node, GameObject parent, List<GameObject> goList)
	{
		...
		while (j < vertices.Length)
		{
			...
			if(vertexPos.x > nMaxXSize)
				nMaxXSize = vertexPos.x;

			if(vertexPos.x < nMinXSize)
				nMinXSize = vertexPos.x;
	...
	private void fillSizeValues(List<GameObject> goList, LSystemGraph node)
	{
