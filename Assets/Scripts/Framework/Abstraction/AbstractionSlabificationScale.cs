...
public class AbstractionSlabificationScale : Abstraction
{
	...
	public AbstractionSlabificationScale()
	{
	...
	public override Representation Process(Representation representation)
	{
		...
		GameObject tempGO = new GameObject (repre.gameObject.name);

		createMesh (result.structure, tempGO, tempList);
		...
		transform (result.gameObject, result.goList, tempList);
		
		result.structure = copyStructure (null, result.structure as LSystemGraph, result.gameObject);
