...
public class AbstractionGraphWarp : Abstraction
{
	...
	public override Representation Process(Representation representation)
	{
		...
		{
			deformedPositions[node.Key] = warpPosition(node.Key, new Vector2(node.Value.position.x, node.Value.position.y));
		}
