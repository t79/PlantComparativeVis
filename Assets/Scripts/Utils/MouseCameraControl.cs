...
public class MouseCameraControl : MonoBehaviour
{
	...
	public MouseControlConfiguration yaw = new MouseControlConfiguration { mouseButton = MouseButton.Right, sensitivity = 1F };
	
	// Pitch default configuration
	public MouseControlConfiguration pitch = new MouseControlConfiguration { mouseButton = MouseButton.Right/*, modifiers = new Modifiers{ leftControl = true }*/, sensitivity = 1F };
	...
	public MouseControlConfiguration verticalTranslation = new MouseControlConfiguration { mouseButton = MouseButton.Left, sensitivity = 5F };
	
	// Horizontal translation default configuration
	public MouseControlConfiguration horizontalTranslation = new MouseControlConfiguration { mouseButton = MouseButton.Left, sensitivity = 5F };
	
	// Depth (forward/backward) translation default configuration
	public MouseControlConfiguration depthTranslation = new MouseControlConfiguration { mouseButton = MouseButton.Left, sensitivity = 5F };
	...
	void LateUpdate ()
	{
		if (yaw.isActivated())
		{
			...
			transform.RotateAround(yaw.target, new Vector3(0, 1f, 0), rotationX);
		}
		if (pitch.isActivated())
		{
			float rotationY = Input.GetAxis(mouseVerticalAxisName) * pitch.sensitivity;
			transform.RotateAround(pitch.target, new Vector3(1f, 0, 0), -rotationY);
